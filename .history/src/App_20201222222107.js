...
function App() {
  const currentHour = new Date().getHours() > 9 ? Date().getHours() : '0'+Date().getHours();
  const currentMinutes = new Date().getHours() > 9 ? Date().getHours() : '0'+Date().getMinutes();
  return (
    <div className="App">
      ...
      <div className={classes.mainContainer}>
        <div className={classes.ProductPreview}>
          ...
          <div className={classes.FeatureData}>
            <p>{`${this.currentHour}:${new Date().getMinutes()}`}</p>
          </div>
        </div>
